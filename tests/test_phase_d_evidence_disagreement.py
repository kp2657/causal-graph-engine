...
def _cb(gene="TNF", program_id="p", cell_type="macrophage", disease="IBD",
...
class TestGeneticsVsPerturbation:
    def test_sign_conflict_detected(self):
        ...
        r = check_genetics_vs_perturbation("TNF", "IBD", -0.4, [cb])
    ...
    def test_no_conflict_same_sign(self):
        from pipelines.evidence_disagreement import check_genetics_vs_perturbation
        cb = _cb(beta=0.6, evidence_tier="Tier1_Interventional")
        assert check_genetics_vs_perturbation("TNF", "IBD", 0.4, [cb]) is None

    def test_nan_mr_gamma_returns_none(self):
        from pipelines.evidence_disagreement import check_genetics_vs_perturbation
        cb = _cb(beta=0.6)
        assert check_genetics_vs_perturbation("TNF", "IBD", float("nan"), [cb]) is None

    def test_zero_mr_gamma_returns_none(self):
        from pipelines.evidence_disagreement import check_genetics_vs_perturbation
        assert check_genetics_vs_perturbation("TNF", "IBD", 0.0, [_cb()]) is None
    ...
    def test_severity_flag_with_multiple_betas(self):
        from pipelines.evidence_disagreement import check_genetics_vs_perturbation
        betas = [_cb(beta=0.5), _cb(cell_type="enterocyte", beta=0.4)]
        r = check_genetics_vs_perturbation("TNF", "IBD", -0.3, betas)
        assert r.severity == "flag"

    def test_severity_warning_with_single_beta(self):
        from pipelines.evidence_disagreement import check_genetics_vs_perturbation
        r = check_genetics_vs_perturbation("TNF", "IBD", -0.3, [_cb(beta=0.5)])
    ...
    def test_program_id_filter(self):
        ...
        r = check_genetics_vs_perturbation("TNF", "IBD", -0.3,
    ...
    def test_nan_beta_excluded(self):
        ...
        assert check_genetics_vs_perturbation("TNF", "IBD", -0.4, [cb_nan]) is None
...
class TestPerturbationVsChemical:
    def test_sign_conflict(self):
        ...
        r = check_perturbation_vs_chemical("TNF", "IBD", 0.6, -0.4)
        assert r is not None
    ...
    def test_no_conflict_same_sign(self):
        from pipelines.evidence_disagreement import check_perturbation_vs_chemical
        assert check_perturbation_vs_chemical("TNF", "IBD", 0.6, 0.4) is None

    def test_zero_betas_returns_none(self):
        from pipelines.evidence_disagreement import check_perturbation_vs_chemical
        assert check_perturbation_vs_chemical("TNF", "IBD", 0.0, 0.4) is None
        assert check_perturbation_vs_chemical("TNF", "IBD", 0.4, 0.0) is None

    def test_nan_betas_returns_none(self):
        from pipelines.evidence_disagreement import check_perturbation_vs_chemical
        assert check_perturbation_vs_chemical("TNF", "IBD", float("nan"), 0.4) is None
...
class TestBulkVsSinglecell:
    def test_strong_bulk_near_zero_sc(self):
        ...
        r = check_bulk_vs_singlecell("TNF", "IBD", 0.45, [cb_tiny])
        assert r is not None
    ...
    def test_weak_lincs_not_triggered(self):
        ...
        assert check_bulk_vs_singlecell("TNF", "IBD", 0.20, [cb_tiny]) is None
...
class TestCrossContextSignFlip:
    def test_sign_flip_detected(self):
        ...
        cb_mac = _cb(cell_type="macrophage", beta=0.6, context_verified=True)
        cb_ent = _cb(cell_type="enterocyte", beta=-0.5, context_verified=True)
        r = check_cross_context_sign_flip("TNF", "IBD", [cb_mac, cb_ent])
        assert r is not None
        assert r.rule == "cross_context_sign_flip"
        assert r.severity == "block"
        assert {r.cell_type_a, r.cell_type_b} == {"macrophage", "enterocyte"}

    def test_same_sign_no_flip(self):
        from pipelines.evidence_disagreement import check_cross_context_sign_flip
        cb_mac = _cb(cell_type="macrophage", beta=0.6, context_verified=True)
        cb_ent = _cb(cell_type="enterocyte", beta=0.4, context_verified=True)
        assert check_cross_context_sign_flip("TNF", "IBD", [cb_mac, cb_ent]) is None

    def test_only_one_context_verified(self):
        from pipelines.evidence_disagreement import check_cross_context_sign_flip
        cb_mac = _cb(cell_type="macrophage", beta=0.6, context_verified=True)
        cb_ent = _cb(cell_type="enterocyte", beta=-0.5, context_verified=False)
        assert check_cross_context_sign_flip("TNF", "IBD", [cb_mac, cb_ent]) is None
    ...
    def test_nan_betas_excluded(self):
        ...
        import math
        cb_mac = _cb(cell_type="macrophage", beta=0.6, context_verified=True)
        cb_nan = _cb(cell_type="enterocyte", beta=float("nan"), context_verified=True)
        assert check_cross_context_sign_flip("TNF", "IBD", [cb_mac, cb_nan]) is None

    def test_program_id_filter(self):
        from pipelines.evidence_disagreement import check_cross_context_sign_flip
        cb_mac = _cb(program_id="p1", cell_type="macrophage", beta=0.6, context_verified=True)
        # Same cell type, different program — deduplicated, so only one per cell type
        cb_ent = _cb(program_id="p2", cell_type="enterocyte", beta=-0.5, context_verified=True)
        r = check_cross_context_sign_flip("TNF", "IBD", [cb_mac, cb_ent], program_id="p1")
        # p2 excluded → only one cell type left → no flip
        assert r is None

    def test_three_cell_types_finds_first_conflict(self):
        from pipelines.evidence_disagreement import check_cross_context_sign_flip
        cb1 = _cb(cell_type="macrophage",  beta=0.6,  context_verified=True)
        cb2 = _cb(cell_type="enterocyte",  beta=0.4,  context_verified=True)
        cb3 = _cb(cell_type="smooth_muscle", beta=-0.3, context_verified=True)
        r = check_cross_context_sign_flip("TNF", "IBD", [cb1, cb2, cb3])
        assert r is not None
        assert r.severity == "block"
...
class TestRunAllDisagreementChecks:
    def test_returns_list(self):
        ...
        records = run_all_disagreement_checks("TNF", "IBD", [])
        assert isinstance(records, list)

    def test_empty_when_no_conflict(self):
        from pipelines.evidence_disagreement import run_all_disagreement_checks
        cb = _cb(beta=0.6, context_verified=False)
        records = run_all_disagreement_checks("TNF", "IBD", [cb], mr_gamma=0.5)
        assert records == []
    ...
    def test_rule4_fires_independently(self):
        ...
        cb_mac = _cb(cell_type="macrophage", beta=0.6, context_verified=True)
        cb_ent = _cb(cell_type="enterocyte", beta=-0.5, context_verified=True)
        records = run_all_disagreement_checks("TNF", "IBD", [cb_mac, cb_ent])
    ...
    def test_multiple_rules_fire(self):
        ...
        records = run_all_disagreement_checks(
            "TNF", "IBD", [cb_mac, cb_ent], mr_gamma=-0.4
        )
        rules = {r.rule for r in records}
    ...
    def test_lincs_triggers_rules_2_and_3(self):
        ...
        cb_tiny = _cb(cell_type="enterocyte", beta=0.01,
                      context_verified=True, evidence_tier="Tier1_Interventional")
        records = run_all_disagreement_checks(
            "TNF", "IBD", [cb_t1, cb_tiny],
    ...
    def test_all_records_are_correct_type(self):
        ...
        cb_mac = _cb(cell_type="macrophage", beta=0.6, context_verified=True)
        cb_ent = _cb(cell_type="enterocyte", beta=-0.5, context_verified=True)
        records = run_all_disagreement_checks("TNF", "IBD", [cb_mac, cb_ent], mr_gamma=-0.3)
        for r in records:
