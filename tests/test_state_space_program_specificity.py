...
Tests for:
  ...
  - compute_beta_program_concentration   (program_specificity.py)
...
def _make_adata(n_disease: int, n_healthy: int, n_genes: int = 20,
                disease_multiplier: float = 3.0) -> "AnnData":
    """Build a minimal AnnData with disease (AD) and healthy cells."""
    try:
        import anndata as ad
    except ImportError:
        raise unittest.SkipTest("anndata not installed")

    n_cells = n_disease + n_healthy
    rng = np.random.default_rng(42)

    # Disease cells have higher expression for genes 0..4 (first half)
    X = rng.uniform(0.1, 1.0, size=(n_cells, n_genes)).astype(np.float32)
    X[:n_disease, :5] *= disease_multiplier  # disease-specific upregulation in first 5 genes

    disease_labels = (
        ["alzheimer's disease"] * n_disease +
        ["normal"] * n_healthy
    )
...
def _make_programs() -> list[dict]:
    """Two programs: one disease-specific (GENE0-4), one generic (GENE5-9)."""
    return [
        {
            "program_id": "ad_neuroinflammation",
            "gene_set": [f"GENE{i}" for i in range(5)],  # disease-upregulated
        },
        {
            "program_id": "generic_housekeeping",
            "gene_set": [f"GENE{i}" for i in range(5, 10)],  # not disease-specific
        },
    ]
...
class TestComputeProgramDiseaseWeights(unittest.TestCase):
    ...
    def test_disease_specific_program_gets_high_weight(self):
        from pipelines.state_space.program_specificity import compute_program_disease_weights
        adata = _make_adata(n_disease=50, n_healthy=50, disease_multiplier=4.0)
        programs = _make_programs()
        weights = compute_program_disease_weights(adata, programs)

        self.assertIn("ad_neuroinflammation", weights)
        self.assertIn("generic_housekeeping", weights)
        # disease-specific program should have higher weight
        self.assertGreater(weights["ad_neuroinflammation"], weights["generic_housekeeping"])
        self.assertGreater(weights["ad_neuroinflammation"], 0.5)
    ...
    def test_generic_program_gets_low_weight(self):
        ...
        programs = _make_programs()
        weights = compute_program_disease_weights(adata, programs)
        # Generic program (no disease-specific upregulation) should get low weight
        self.assertLess(weights["generic_housekeeping"], 0.3)

    def test_weights_bounded_zero_one(self):
        from pipelines.state_space.program_specificity import compute_program_disease_weights
        adata = _make_adata(n_disease=50, n_healthy=50, disease_multiplier=10.0)
        programs = _make_programs()
        weights = compute_program_disease_weights(adata, programs)
    ...
    def test_too_few_cells_returns_uniform(self):
        from pipelines.state_space.program_specificity import compute_program_disease_weights
        adata = _make_adata(n_disease=5, n_healthy=3)  # below _MIN_CELLS_PER_GROUP=10
        programs = _make_programs()
        weights = compute_program_disease_weights(adata, programs)
    ...
    def test_gene_loadings_mode(self):
        ...
        programs = [
            {
                "program_id": "weighted_disease",
                "gene_loadings": {f"GENE{i}": float(i + 1) for i in range(5)},
            }
        ]
        weights = compute_program_disease_weights(adata, programs)
        self.assertIn("weighted_disease", weights)
        self.assertGreater(weights["weighted_disease"], 0.5)


# ---------------------------------------------------------------------------
# compute_beta_program_concentration
# ---------------------------------------------------------------------------

class TestComputeBetaAmdConcentration(unittest.TestCase):

    def test_all_in_disease_programs(self):
        from pipelines.state_space.program_specificity import compute_beta_program_concentration
        beta = {
            "ad_neuroinflammation": {"beta": 0.8},
            "generic_housekeeping": {"beta": 0.1},
        }
        weights = {"ad_neuroinflammation": 1.0, "generic_housekeeping": 0.0}
        conc = compute_beta_program_concentration(beta, weights)
        # 0.8 / (0.8 + 0.1) ≈ 0.889
    ...
    def test_uniform_beta_low_concentration(self):
        ...
        conc = compute_beta_program_concentration(beta, weights)
        # Only 1/10 of β-mass in disease-specific program
        self.assertAlmostEqual(conc, 0.1, places=2)

    def test_nan_beta_skipped(self):
        from pipelines.state_space.program_specificity import compute_beta_program_concentration
        beta = {"prog_disease": {"beta": float("nan")}, "prog_gen": {"beta": 0.5}}
        weights = {"prog_disease": 1.0, "prog_gen": 0.0}
        conc = compute_beta_program_concentration(beta, weights)
        self.assertEqual(conc, 0.0)  # NaN skipped, only generic with w=0

    def test_empty_inputs(self):
        from pipelines.state_space.program_specificity import compute_beta_program_concentration
        self.assertEqual(compute_beta_program_concentration({}, {}), 0.0)
        self.assertEqual(compute_beta_program_concentration({"p": {"beta": 1.0}}, {}), 0.0)
