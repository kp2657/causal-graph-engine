...
def program_loadings_tnf():
    ...
    return [
        ProgramLoading(gene="TNF", program_id="AMD_rpe_P00", cell_type="retinal_pigment_epithelial",
                       disease="AMD", nmf_loading=0.6, transition_de_signal=0.8,
...
def conditional_betas_tnf():
    ...
    return [
        ConditionalBeta(gene="TNF", program_id="AMD_rpe_P00", cell_type="retinal_pigment_epithelial",
                        disease="AMD", beta=0.7, context_verified=True,
...
def conditional_gammas_ibd():
    ...
    return {
        "AMD_rpe_P00": ConditionalGamma(
            program_id="AMD_rpe_P00", trait="AMD", disease="AMD",
...
class TestRedirectionPerCelltype:
    def test_positive_score_for_good_target(
        ...
        result = compute_therapeutic_redirection_per_celltype(
            gene="TNF", disease="AMD", cell_type="retinal_pigment_epithelial",
    ...
    def test_zero_score_when_no_transition_matrix(
        ...
        result = compute_therapeutic_redirection_per_celltype(
            gene="CFH", disease="AMD", cell_type="retinal_pigment_epithelial",
            program_loadings=program_loadings_tnf,
            conditional_betas=conditional_betas_tnf,
            conditional_gammas=conditional_gammas_ibd,
    ...
    def test_nan_beta_excluded(
        ...
        from models.latent_mediator import ConditionalBeta
        nan_betas = [ConditionalBeta(gene="TNF", program_id="AMD_rpe_P00",
    ...
    def test_pooled_fraction_computed(
        ...
        from models.latent_mediator import ConditionalBeta
        pooled_betas = [ConditionalBeta(gene="TNF", program_id="AMD_rpe_P00",
                                         cell_type="retinal_pigment_epithelial", disease="AMD",
...
class TestComputeTherapeuticRedirection:
    def test_equal_weight_average(self):
        ...
        data = {
            "retinal_pigment_epithelial": {"redirection": 0.4, "n_programs": 1,
                           "pooled_fraction": 0.0, "evidence_tiers": ["T2"], "provenance": ["p"]},
            "choroidal_endothelial": {"redirection": 0.2, "n_programs": 1,
                           "pooled_fraction": 0.0, "evidence_tiers": ["T3"], "provenance": ["q"]},
        }
        result = compute_therapeutic_redirection("CFH", "AMD", data)
    ...
    def test_custom_weights(self):
        from pipelines.state_space.therapeutic_redirection import compute_therapeutic_redirection
        data = {
            "retinal_pigment_epithelial": {"redirection": 1.0, "n_programs": 1, "pooled_fraction": 0.0,
                           "evidence_tiers": [], "provenance": []},
            "choroidal_endothelial": {"redirection": 0.0, "n_programs": 0, "pooled_fraction": 0.0,
                           "evidence_tiers": [], "provenance": []},
        }
        result = compute_therapeutic_redirection("CFH", "AMD", data,
                                                  cell_type_weights={"retinal_pigment_epithelial": 2.0, "choroidal_endothelial": 1.0})
        # retinal_pigment_epithelial weight = 2/3, choroidal_endothelial = 1/3 → 1.0×(2/3) + 0.0×(1/3) ≈ 0.667
        assert abs(result.therapeutic_redirection - 2/3) < 1e-9

    def test_context_confidence_warning_above_threshold(self):
        from pipelines.state_space.therapeutic_redirection import compute_therapeutic_redirection
        data = {
            "retinal_pigment_epithelial": {"redirection": 0.5, "n_programs": 2,
    ...
    def test_returns_therapeutic_redirection_result(self):
        ...
        data = {"m": {"redirection": 0.0, "n_programs": 0, "pooled_fraction": 0.0,
                      "evidence_tiers": [], "provenance": []}}
        result = compute_therapeutic_redirection("X", "AMD", data)
