...
class TestEstimateBetaNewTiersFallback(unittest.TestCase):
    ...
    def test_burden_above_lincs(self):
        """Rare burden (Tier2rb) activates before LINCS when no eQTL/pQTL."""
        from pipelines.ota_beta_estimation import estimate_beta
        burden = {
            "burden_beta": 0.7,
            "burden_p": 1e-6,
        ...
        lincs_sig = {"GENEABC": {"log2fc": 0.5}}  # would activate Tier3
        result = estimate_beta(
            gene="HMCN1",
            program="ECM_remodeling",
            lincs_signature=lincs_sig,
            program_gene_set={"GENEABC"},
            burden_data=burden,
            program_loading=0.6,
        )
        self.assertEqual(result["evidence_tier"], "Tier2rb_RareBurden")
...
class TestEqtlCatalogueServerHelpers(unittest.TestCase):
    ...
    def test_disease_cell_type_map_coverage(self):
        ...
        required = {"CAD", "RA", "IBD", "AMD", "AD", "T2D", "MS", "T1D", "SLE"}
        self.assertTrue(required.issubset(set(DISEASE_SC_EQTL_CELL_TYPES.keys())))

    def test_pqtl_key_genes_amd(self):
        from mcp_servers.eqtl_catalogue_server import DISEASE_KEY_PQTL_PROTEINS
        amd_proteins = DISEASE_KEY_PQTL_PROTEINS.get("AMD", {})
        # CFH is the most important pQTL target for AMD
        self.assertIn("CFH", amd_proteins)
        self.assertIn("C3", amd_proteins)
        self.assertIn("CFB", amd_proteins)
...
class TestUkbWesServerHelpers(unittest.TestCase):
    ...
    def test_disease_burden_efo_coverage(self):
        ...
        required = {"AMD", "CAD", "RA", "IBD", "AD", "T2D"}
    ...
    def test_schema_pqtl_amd_fields_present(self):
        from graph.schema import DISEASE_CELL_TYPE_MAP
        amd = DISEASE_CELL_TYPE_MAP.get("AMD", {})
        self.assertIn("pqtl_study_priority", amd)
        self.assertIn("pqtl_key_genes", amd)
        self.assertIn("sc_eqtl_cell_types", amd)
        self.assertIn("CFH", amd["pqtl_key_genes"])
        self.assertIn("C3", amd["pqtl_key_genes"])
