...
# V8: Use target -> evidences (the ONLY correct path for evidence rows in v4)
q = """
query DiseaseGeneEvidences($ensemblId: String!, $efoId: String!) {
  target(ensemblId: $ensemblId) {
    evidences(efoIds: [$efoId], datasourceIds: ["gwas_credible_sets"]) {
      rows {
        score
        credibleSet {
          beta
...
"""
test_query("DiseaseGeneEvidences_v8", q, {"ensemblId": "ENSG00000169174", "efoId": "EFO_0001645"})
